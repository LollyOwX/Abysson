...
| Libro: input — mouse + tastiera | `GamePanel` (`cycleBookIndex()`, `cycleBookZone()`), `KeyHandler` (ramo `bookState`), `UI` (`handleBookClick()`) | Click su un bookmark = `selectBookIndex()` (tramite `Button`, vedi riga sotto). Da tastiera: **A/D** = area generale precedente/successiva (`cycleBookIndex`, clamp ai bordi come le pagine, apre la prima/ultima se nessuna è ancora selezionata), **W/S** = voce precedente/successiva dentro all'area generale corrente (`cycleBookZone`, limite = quante voci sono effettivamente sbloccate in quell'area — nessuna UI cliccabile per questo, solo tastiera per ora), **LEFT/RIGHT** = pagina (`turnBookPage`, limite = `BookEntry.pages.length` della voce corrente). Tutti passano da `startBookTransition()`, quindi qualunque cambio (mouse o tastiera) fa sempre l'animazione voltapagina. **ESC chiude tutto il libro in un colpo solo** (non risale di un livello alla volta) — nota, non ancora richiesto esplicitamente. |
| `Button.java` — elemento cliccabile riusabile | `main/Button.java`, `UI.java` (`bookmarkButtons[]`) | Aggiunto il 01/08: area (`Rectangle bounds`) + azione (`Runnable onClick`), pensato per essere riusato ovunque serva un bottone — non solo i bookmark, in futuro anche il contenuto delle pagine (icone, mostri, oggetti...) invece di liste di testo generate al volo. `setBounds()` va richiamato ogni frame (la posizione dipende dallo scaling del libro), `click(x, y)` esegue l'azione se il punto è dentro. I 6 bookmark sono `bookmarkButtons[i]`, creati nel costruttore di `UI` con `() -> gp.selectBookIndex(i+1)` come azione. **Hover non ancora gestito** — rimosso un tentativo abbozzato il 01/08, da riprendere più avanti. La precedente lista di bottoni testuali per le voci (bookzone) è stata tolta: le voci si scelgono solo da tastiera (W/S) finché non si decide come renderle cliccabili sul contenuto vero della pagina. |
