...
| Libro (quest/inventario/statistiche) | `GamePanel.bookState`, `drawBook()`, `turnBookPage()` | Tasto **I** in gioco → cinematic `Open_book.gif` → atterra su `bookState`, disegna `res/ui/book.png` a schermo intero. **ESC/I** richiude. **LEFT/RIGHT** chiamano `turnBookPage()`: overlay `Turning_pages_left/right.gif` (asset confermati esistenti in `res/cinematics/`) + `pageIndex` (0=Quest, 1=Inventario, 2=Statistiche) che si muove **a clamp** (si ferma a inizio/fine, niente wrap — come un libro vero). "Bookmark" è terminologia riservata al futuro macro-menu (vedi TODO), non va confusa col `pageIndex` attuale. **Contenuto per pagina non ancora implementato** — TODO in `drawBook()` esattamente dove va aggiunto. |
...
- [ ] Implementare il contenuto vero delle pagine del libro (Quest/Inventario/Statistiche, `pageIndex` 0/1/2) — TODO segnato direttamente in `GamePanel.drawBook()`.
...
- [ ] Aggiungere i "segnalibri" (macro-menu: Quest vs Inventario vs Statistiche vs altro) sopra all'attuale `pageIndex`, che resterà la navigazione dentro al singolo segnalibro (micro-menu) — concetto ancora da progettare, tenere i due livelli distinti.
