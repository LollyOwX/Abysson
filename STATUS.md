...
| Libro: input — mouse + tastiera | `GamePanel` (`cycleBookIndex()`, `cycleBookZone()`), `KeyHandler` (ramo `bookState`), `UI` (`handleBookClick()`) | Click su un bookmark = `selectBookIndex()`. Da tastiera: **A/D** = area generale precedente/successiva (`cycleBookIndex`, clamp ai bordi come le pagine, apre la prima/ultima se nessuna è ancora selezionata), **W/S** = voce precedente/successiva dentro all'area generale corrente (`cycleBookZone`, limite = quante voci sono effettivamente sbloccate in quell'area, non più una costante fissa), **LEFT/RIGHT** = pagina (`turnBookPage`, limite = `BookEntry.pages.length` della voce corrente). Tutti e tre passano da `startBookTransition()`, quindi qualunque cambio (mouse o tastiera) fa sempre l'animazione voltapagina. **ESC chiude tutto il libro in un colpo solo** (non risale di un livello alla volta) — nota, non ancora richiesto esplicitamente. |
...
- [ ] `bookzone` (voce dentro all'area generale) non ha ancora una UI cliccabile — `GamePanel.selectBookZone()` esiste ma nessun bottone/asset la aziona ancora, solo tastiera (W/S).
