...
| Libro: navigazione a 3 livelli | `GamePanel` (`bookindex`/`bookzone`/`bookpage`, costanti `bookindex_map=1`…`bookindex_inventory=6`, `selectBookIndex()`, `selectBookZone()`, `turnBookPage()`, `cycleBookIndex()`, `cycleBookZone()`, `startBookTransition()`, `getCurrentBookEntry()`), `BookEntry.java`, `UI.drawBookScreen()`, `UI.getBookContentText()` | Progettata il 28/07 nello stesso stile di `gameState` (costanti nominate + variabile), **scelta esplicita al posto di enum+stack**: la profondità di navigazione è fissa a 3 livelli (mai variabile), quindi uno stack non aggiungerebbe nulla, solo cerimonia. **`bookindex`** = area generale (bookmark: 6 costanti nominate `bookindex_map=1` … `bookindex_inventory=6`, valore `0` = nessuna selezionata, array/liste indicizzate con `bookindex - 1`). **`bookzone`**/**`bookpage`** (29/07, ristrutturato da placeholder fisso a dati): non più costanti `BOOKZONE_COUNT`/`MICRO_PAGE_COUNT` uguali per tutti — ora `GamePanel.unlockedBookZones` è una `List<List<BookEntry>>` (una lista per area, indice `bookindex - 1`), stesso pattern di `Entity.unlockedAbilities`: una voce esiste per il giocatore solo se è nella lista, niente booleano "sbloccato/bloccato" da controllare altrove. Ogni `BookEntry` ha `name` + `pages[]` a lunghezza variabile (risolve il "non sempre presenti": una voce senza pagina requisiti ha semplicemente un array più corto). `bookzone` è l'indice dentro la lista dell'area attiva, `bookpage` l'indice dentro `pages[]` della voce corrente — presi con `GamePanel.getCurrentBookEntry()` (torna `null` se l'area non ha ancora nessuna voce sbloccata). Attenzione ai nomi metodo, seguono il CAMPO che impostano: `selectBookIndex(int)` imposta `bookindex`, `selectBookZone(int)` imposta `bookzone`. Ogni cambio passa da `startBookTransition()`, che innesca **sempre** l'animazione `Turning_pages_left/right.gif`. Sfondo: `book.png` se `bookindex == 0`, altrimenti `book_X.png` dell'area attiva (bookmark evidenziato già dentro l'immagine, niente overlay separato). `UI.getBookContentText()` ora è solo una lettura di `BookEntry` (niente più if/else-if a 24 rami): `"Niente sbloccato ancora in quest'area"` se `getCurrentBookEntry()` torna null. **Punti di sblocco veri non ancora collegati** — `GamePanel.seedBookDebugEntries()` (chiamata nel costruttore) semina una voce di debug per area solo per provare la navigazione, da rimuovere quando arrivano i contenuti veri e i reali punti di sblocco (quest accettata, mostro incontrato, area scoperta...). |
| Libro: input — mouse + tastiera | `GamePanel` (`cycleBookIndex()`, `cycleBookZone()`), `KeyHandler` (ramo `bookState`), `UI` (`handleBookClick()`, `bookZoneButtonBounds`) | Click su un bookmark = `selectBookIndex()`. Click su una voce (bottone testuale, vedi riga sotto) = `selectBookZone()`. Da tastiera: **A/D** = area generale precedente/successiva (`cycleBookIndex`, clamp ai bordi come le pagine, apre la prima/ultima se nessuna è ancora selezionata), **W/S** = voce precedente/successiva dentro all'area generale corrente (`cycleBookZone`, limite = quante voci sono effettivamente sbloccate in quell'area), **LEFT/RIGHT** = pagina (`turnBookPage`, limite = `BookEntry.pages.length` della voce corrente). Tutti passano da `startBookTransition()`, quindi qualunque cambio (mouse o tastiera) fa sempre l'animazione voltapagina. **ESC chiude tutto il libro in un colpo solo** (non risale di un livello alla volta) — nota, non ancora richiesto esplicitamente. |
| Libro: bottoni voci (bookzone) | `UI.java` (`bookZoneButtonBounds`, `ZONE_BUTTON_PAD_X/Y`, `ZONE_BUTTON_GAP`) | Nessun asset dedicato per le voci (a differenza dei bookmark, che hanno `book_X.png`), quindi bottoni testuali disegnati e dimensionati **sul proprio testo** (font metrics + padding), non su un layout fisso — lista verticale nell'angolo alto-sinistro della pagina, zona libera perché i bookmark occupano la fascia centrale (`BOOKMARK_IMAGE_RECTS` parte da y=73 in spazio immagine). Lunghezza dinamica: tante voci quante sono effettivamente sbloccate (`GamePanel.unlockedBookZones.get(bookindex-1).size()`), non un numero fisso. Con poche voci va bene; se un'area ne avrà molte servirà scroll o paginazione, non c'è ancora gestione per quel caso. |
| Libro: hit-test mouse bookmark | `UI.java` (`BOOKMARK_IMAGE_RECTS`, `bookmarkScreenBounds`, `updateBookMouseHover()`, `handleBookClick()`) | Stesso schema già usato per il menu titolo (`menuItemBounds`/`updateMouseHover`/`handleTitleClick`): rettangoli calcolati ogni frame in coordinate schermo (dipendono dallo scaling del libro), poi hit-test su hover/click. `BOOKMARK_IMAGE_RECTS` sono le 6 aree generali (in coordinate immagine 272×272) misurate confrontando i pixel non trasparenti di `book.png` con ciascun `book_X.png`; indice array = `bookindex - 1` (stesso ordine di `ZONE_IMAGES`: map, quests, skills, calendar, bestiary, inventory). Il mouse handler globale in `GamePanel` (già esistente per il titolo) inoltra anche a `updateBookMouseHover`/`handleBookClick`, che si autolimitano a `gameState == bookState`. |
...
- [ ] Collegare i veri punti di sblocco (quest accettata, mostro incontrato, area scoperta...) ad `unlockedBookZones`, e rimuovere `GamePanel.seedBookDebugEntries()` (voci di debug, vedi §2). Il contenuto testuale in `UI.getBookContentText()` legge già da `BookEntry`, quindi basta popolare le liste con dati veri — non serve più toccare quel metodo.
