...
| Libro: apertura/pagine | `GamePanel.bookState`, `UI.drawBookScreen()` | Tasto **I** in gioco → cinematic `Open_book.gif` → atterra su `bookState`. Sfondo disegnato con **aspect ratio corretto** (scala uniforme `Math.min(screenW/imgW, screenH/imgH)` + centratura, stessa formula già usata per le cinematic) — **non più stirato** a `screenWidth×screenHeight` (bug del 27/07, vedi §3). **ESC/I** → `GamePanel.closeBook()` (richiude e resetta zona/sezione/pagina a 0). |
| Libro: navigazione a 3 livelli | `GamePanel` (`currentZone/currentIndex/currentPage`, `selectBookZone()`, `selectBookIndex()`, `turnBookPage()`, `startBookTransition()`), `UI.drawBookScreen()` | Aggiunto il 27/07. **Zona** (macrozona, 6 bookmark: Quest/Inventario/Mappa/Abilità/Calendario/Bestiario) → **Index** (sezioni sotto al bookmark attivo, solo testo placeholder) → **Pagina** (microzona, LEFT/RIGHT come prima). Ogni cambio a qualunque livello passa da `startBookTransition()`, che innesca **sempre** l'animazione `Turning_pages_left/right.gif` (stesso meccanismo già esistente per le pagine, ora riusato/generalizzato) — nessuno swap istantaneo. Sfondo: `book.png` se `currentZone == -1` (nessuna zona scelta), altrimenti `book_X.png` della zona attiva (bookmark evidenziato "dentro" l'immagine stessa — vedi `GamePanel.getCurrentBookImage()`). I bookmark sono cliccabili col mouse (hit-test in `UI` — vedi sotto); l'index si clicca sotto al bookmark attivo. **Contenuto reale non ancora implementato**: solo scheletro di navigazione + testo placeholder (`"Zona > Sezione X — Pagina Y/Z"`), vedi TODO. |
| Libro: hit-test mouse bookmark/index | `UI.java` (`BOOKMARK_IMAGE_RECTS`, `bookmarkScreenBounds`, `indexItemBounds`, `updateBookMouseHover()`, `handleBookClick()`) | Stesso schema già usato per il menu titolo (`menuItemBounds`/`updateMouseHover`/`handleTitleClick`): rettangoli calcolati ogni frame in coordinate schermo (dipendono dallo scaling del libro), poi hit-test su hover/click. `BOOKMARK_IMAGE_RECTS` sono le 6 zone (in coordinate immagine 272×272) misurate confrontando i pixel non trasparenti di `book.png` con ciascun `book_X.png`. Il mouse handler globale in `GamePanel` (già esistente per il titolo) ora inoltra anche a `updateBookMouseHover`/`handleBookClick`, che si autolimitano a `gameState == bookState`. |
...
- [ ] Implementare il contenuto vero delle pagine del libro per ogni Zona/Index/Pagina — al momento solo scheletro di navigazione + testo placeholder (`"Zona > Sezione X — Pagina Y/Z"`) in `UI.drawBookScreen()`. `INDEX_COUNT`/`MICRO_PAGE_COUNT` in `GamePanel` sono placeholder uguali per tutte le zone (2 e 2) — quasi certamente da rendere specifici per zona quando i contenuti veri saranno pronti (es. Quest potrebbe avere più sezioni di Calendario).
- [ ] Asset `res/ui/bookmarks.png` (134×16, striscia dei 6 bookmark "a riposo") ed `extended_bookmarks*.png` (134×86 tutti insieme, 14×83 i singoli) sono presenti nel repo ma **non ancora usati** nel codice — al momento l'evidenziazione del bookmark attivo è "dentro" l'immagine `book_X.png` stessa, non un overlay separato. Da valutare se/come integrarli (es. animazione di hover sul bookmark, simile al glow del menu titolo).
- [ ] Nessuna navigazione da tastiera per Zona/Index (solo mouse per ora) — solo LEFT/RIGHT per la pagina dentro la sezione corrente restano da tastiera. Da aggiungere se serve accessibilità/controller.
